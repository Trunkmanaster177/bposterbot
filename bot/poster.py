...
def _upload_images(page, image_urls: list):
    """Upload images — handles both local file paths and URLs."""
    from playwright.sync_api import TimeoutError as PWTimeout

    temp_files = []
    for item in image_urls[:9]:
        try:
            # Local file path (e.g. chart PNG generated by matplotlib)
            if os.path.exists(item):
                temp_files.append(item)
                print(f"[poster] Using local file: {item}")
                continue

            # URL — download to temp file
            ext = item.split("?")[0].split(".")[-1].lower()
            ...
            print(f"[poster] Downloaded image: {item[-50:]}")
        except Exception as e:
